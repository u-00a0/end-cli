...
pub enum CatalogBuildError {
    ...
    #[error("recipe facility id {facility_id} must be a machine, got {kind:?}")]
    RecipeFacilityMustBeMachine { facility_id: u32, kind: FacilityKind },
    ...
    #[error("machine facility `{key}` requires power_w")]
    MachineFacilityMissingPower { key: Key },
    #[error("thermal bank facility `{key}` must not define power_w")]
    ThermalBankFacilityHasPower { key: Key },
